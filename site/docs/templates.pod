...
that your are getting all of the layout of a child page embedded into
the parent page, then it's a template problem, not an article data
problem.
...
F<cart_base.tmpl>, F<checkout_base.tmpl> and
F<checkoutfinal_base.tmpl> are used to generated menus and other
generic content into F<cart.tmpl>, F<checkout.tmpl> and
F<checkoutfinal.tmpl> respectively.

These are the shopping cart, checkout page and completed order pages
respectively, used by F<cgi-bin/shop.pl>.

=item *

F<shopitem.tmpl> is hard-wired as the name of the template used to
format products.
...
F<edit_I<level>.tmpl>, you shouldn't need to modify these, unless you
need to limit what your site administrators can do.
...
cellpadding, and font size, or raw html table attributes.
...
The text is formatted with bullets.
