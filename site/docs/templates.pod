...
=item ifUserCanSee I<which>

Check if the currently logged on user can see the given article.

=item iterator ... dynlevel1s

=item dynlevel1 I<field>

Iterate over visible level 1 articles.

=item iterator ... dynlevel2s

=item dynlevel2 I<field>

Iterate over visible level 2 articles that are children of the current
dynlevel1.

=item iterator ... dynlevel3s

=item dynlevel3 I<field>

Iterate over visible level 3 articles that are children of the current
dynlevel2.

=item iterator ... dynallkids_of

=item dynofallkid I<field>

Iterate over visible children and stepchildren of the given articles.

=item iterator ... dynchildren_of

=item dynofchild I<field>

Iterate over visible direct children of the given articles.

=item iterator ... dyncart

=item dyncartitem I<field>

Iterate over the user's shopping cart.  dyncartitem fields include
units, options, extended and all product fields.

=item dyncarttotalcost

Total cost off all items in the user's shopping cart, in cents.

=item dyncarttotalunits

Total number of items in the user's shopping cart.

=item ifUserMemberOf I<groupname>
...
=item thumbimage I<geometry> I<image>

=item thumbimage I<geometry> I<image> I<field>

=item gthumbimage I<geometry> I<image>

=item gthumbimage I<geometry> I<image> I<field>

Inserts a thumbnailed version of the given image, or a field from the
image if the I<field> parameter is supplied.

I<image> can either be an image name, '-' for the current image in the
images or gimages iterator or an image number for thumbimage.
...
An estimate in days for delivery. May look like "1" or "1 or 2" if it is
available at all.
