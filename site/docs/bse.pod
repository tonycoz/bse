...
treat the whole product code as a word for indexing and allow the
configuration to control how words are parsed from the source text.
...
static pages now generate and link directly to their cachec thumbnail
images instead of linking to thumb.pl.
...
you can now configure BSE to have users fetch files from Amazon S3 or
any service accessible by FTP.

=item *

added link aliases to articles, which are used to generate cleaner
URLs if present.

=item *

extras.txt now uses the pregeneration processing, and you can
configure a pregeneration setting section to store relative to the
content directory instead of the template directory.

=item *

shop.fcgi was failing to encrypt emails, due to conflicts between
fastcgi and stdin/out
...
the various thumbimage tags now look in [thumb classes].geometryid to
set the class of the generated img tag.

=item *

added thumbimage[], gthumbimage[] body text tags, and thumbimage,
gthumbimage template tags.
...
the images iterator will no longer prevent automatic image insertion
by itself, there must be an actual image tag replaced.
...
This requires that the user have the C<bse_members_confirm> right and
...
some article iterators now allow their results to be filtered:
allkids_of, allkids_of2, allkids_of3, all dynamic article iterators.

=item *

split BSE::Request into a base and live version, and added a test
derived class.
...
the shop cart action now accepts an embed parameter to specify an
alternate template, this can be used for ajax, though there's no
template support provided in the base template set (adriann)
...
better speed for ajax implementations (not in the base template set),
including a fast CGI version of the shop script.
...
the search template was being generated as a static template, changed
to generate as a dynamic template.
...
use the faster scaling parameter in Imager for thumbnails, if
available
...
Example script: getpcode.pl - extracts a product code from the body of
a product and sets the products product_code.

=item *

Example script: update_title_summary.pl - updates old order items with
the title and description (previously summary) from their associated
products.
...
allow the archive flag on the send form to override the newsletter's
setting
...
avoid infinite redirects on the admin side when the configured admin
base URL doesn't match the name the server reports

=item *

modification to the session summary lists (date/location display) for
seminars (from Adrian)
...
tags using the cgi or config values weren't displaying correctly on
the error page.
...
correctly fill in old alt/url/name values on a failed save of image
properties
...
the location display in the shop code only accepted single digit
location ids.
...
logon.pl now uses the common response output code, to make sure it's
working the same way everything else does.

=item *

renamed the article parameter supplied to tag_url in
BSE::Dynamic::Article to prevent a warning.

=item *

the session_popup tag on the edit booking page now uses a different
...
adding an item to an empty cart now causing the current session cookie
to be set on the non-secure side as well, to ensure that the cart
state is in sync on both secure and non-secure URLs
...
added addsingle\d+ target to shop.pl to allowing individual items from
the catalog page.
...
reload the article earlier when updating dynamic status on
descendants.  The cached dynamic status was being lost when the
article was reloaded later.

=item *

the shop now refreshes to the cart display instead of displaying the
cart in the same request.  The change in add adding to the quantities
in the cart meant that a reload by the client would modify the cart.

The refresh location can be overridden with the r parameter.

=item *

the catalog url tag override wasn't passing through "null"
replacements correctly, producing multiple site URLs at the front of
url tags that were finally processed at dynamic replacement time.

=item *

implement a ifStepAncestor tag which checks if a given article or
article id is the current article or a step/normal ancestor of the
current article.  This is limited to searching to 10 levels of
ancestry.

Note: if you have many step parent links on your site then this tag
can be very slow, since a large number of articles will be loaded
searching for stepparents.
...
allowing the admin to configure the message.

=item *

add a new shop.pl action a_addmultiple which accepts multiple
quantities and adds them to the cart.

=item *

modify the shop add action to add items to the cart rather than
replacing them.  So if product A is in the cart at 10 units and it
added with 1 unit it will become 11 units instead of 1.
